...
class RouteTrie:
    ...
    def insert(self, paths, handler):
        ...
        current_path = self.next
        paths = paths.split('/')
        for path in paths:
            current_path.insert(RouteTrieNode(path))
            current_path = current_path.next[path]
...
class Router:
    ...
    def add_handler(self, path, path_handler):
        ...
        if path == self.root:
            ...
            for sub_path in paths:
                ...
                if x:
                    x = 0
                    current_path[sub_path] = current_path.get(sub_path, RouteTrieNode(sub_path))
                    current_path = current_path[sub_path]
                else:
                    current_path.insert(sub_path, path_handler)
    ...
    def lookup(self, path):
        ...
        for sub_path in paths:
            if sub_path:
                if sub_path in current_path.next:
                    current_path = current_path.next
                    current_path = current_path[sub_path]
                else:
